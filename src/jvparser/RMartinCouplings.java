...
public class RMartinCouplings extends Analyzer {
	...
	private class BasicMetric extends Metric {
		...
		/**
		 * Calculate ce/(ca+ce) Note that Ce+Ca can never be 0 because Ce >=1 by
		 * the decision based on the fact that all classes are at least
		 * dependent on java.lang.Object, even though java.lang stuff is not
		 * being collected, the efferent collector adds one to its search result
		...
		/**
		 * Find all classes outside the given package that depend on things
		 * inside this package Note that nothing outside the default package can
		 * depend on things inside it, so the entire calculation is skipped. Not
		 * doing so causes a problem in 3.0M4; the search engine barfs on the
		...
		/**
		 * Create a search scope consisting of all source packages in the
		 * element's project and all referencing projects. Exclude the element
		 * itself. This results in a DRAMATIC performance increase over
		...
		/**
		 * Uses the JDT search engine to collect all classes outside this
		 * package that depend on things inside this package, in other words,
		 * collect references to this package in the entire workspace, and do
		 * not count those found inside this package
		 * 
		 * @author Frank Sauer
		 * 
		 */
