...
class linePoint
{
  ...
  color[] colorPointArray = {
    #FF8D00, #339045, #E04B50, #DE1FCB, #BC9B52, #3EA587, #961BBC, #1B9DBC, #46413C, #1BBC36, #BCAF1B, #BC871B, #BC361B, #46413C, #C9E37C
  };
  ...
  //void keepWindow()
  //{
  //if (linePoint.size()>(window+1)) removePoint(0);
  //}
  /////////////////////////////////////////////////////////
  void checkMinMax()
  {
    float minValueY=1000000000;
    float maxValueY=-1000000000;
    float minValueX=1000000000;
    float maxValueX=-1000000000;

    pointDef p;
    float val;
    for (int i=0; i<linePoint.size(); i++)
    {
      p = (pointDef )linePoint.get(i);
      val=p.getY();
      p.resetMinMax();
      if (val >= maxValueY)
      {
    ...
    if (minPosY!=maxPosY)
    {
      p=(pointDef )linePoint.get(maxPosY);
      p.setMax();
      p=(pointDef )linePoint.get(minPosY);
      p.setMin();
    }

    if (minPosY!=maxPosY)
    {
      p=(pointDef )linePoint.get(maxPosX);
      p.setMax();
      p=(pointDef )linePoint.get(minPosX);
      p.setMin();
    }
    /*    println("max: "+maxValue+", pos:"+maxPos);
     println("min: "+minValue+", pos:"+minPos);*/
  }
  /////////////////////////////////////////////////////////
  ...
  boolean getMax(int p)
  {
    ...
    return ppoint.getMax();
  }
  /////////////////////////////////////////////////////////
  boolean getMin(int p)
  {
    if (p>linePoint.size()-1 || p<0) 
    {
      println("element "+str(p)+" of line linePoint not FOUND!");
...
class ADLinechart
{
  ...
  void setParameters(boolean vTitle, String title, int tSize, int tOffset, boolean xlVisible, String xlText, int xlSize, int xlOffset, int xnSpacing, boolean ylVisible, String ylText, int ylSize, int ylOffset, boolean lgVisible, int lgSize, int xlgOffset, int ylgOffset, boolean xnVisible, boolean ynVisible, boolean sh, int gx, int gy, int xOf, int yOf, int xM, int yM, int w, int h, int samples, int pr, int lw, int lt)
  {
    ...
    {
      setPointRadius(pRadius, i);
      setLineWidth(lWidth, i);
      setUnderCurveTransparecy((int )lTransp, i);
    }
  ...
  void arrayInit(String textID)
  {
    ...
    currentDataSet.setWindow(lNsamples);
  }
  /////////////////////////////////////////////////////////
  int addDataSet(String textID)
  {
    ...
    currentDataSet=(linePoint )dataSetList.get(dataSetList.size()-1);
    currentDataSet.setWindow(lNsamples);
  ...
  void setSamples(int samples)
  {
    ...
    for (int ds=0; ds<dataSetList.size(); ds++)
    {
      ...
      currentDataSet.setWindow(samples);

      lNsamples = currentDataSet.getSize();
  ...
  void drawXValues()
  {
    ...
    currentDataSet=(linePoint )dataSetList.get(0);
    float xmax = currentDataSet.getXPoint(currentDataSet.getSize()-1);
    float scale = (float)xNumbersSpacing/lNsamples;
    ...
    for (float i=0;i<=1;i+=scale) {
      ...
      pushMatrix();
      translate(x+viewPortW*i, xNumbersyPos-yOffset);
      rotate(PI/2);
      text(str(round2nDecimals(xmax*i, 1)), 0, 0);
      popMatrix();
    }
  }  
  /////////////////////////////////////////////////////////
  ...
  void pushValue(float yValue, float xValue, int dataSet)
  {
    ...
    if (currentDataSet.getSize()==1 && dataSet==0)
    {
      setMin(yValue-0.01);
      setMax(yValue+0.01);
    }
  ...
  /////////////////////////////////////////////////////////
  void drawMinMax(float v, float x, int ds, int pos)
  {
    linePoint currentDataSet =(linePoint )dataSetList.get(ds);

    if (!currentDataSet.visiblePoint) return;

    if (currentDataSet.getMax(pos) || currentDataSet.getMin(pos))
    {
      /*      stroke(currentDataSet.pointColor);
       noFill();
       ellipse(x,translatedValue(v),currentDataSet.pointRadius+8,currentDataSet.pointRadius+8);    */
      noStroke();
      fill(currentDataSet.pointColor, currentDataSet.pointTransparency);
      ellipse(x, translatedValue(v), currentDataSet.pointRadius+8, currentDataSet.pointRadius+8);
    }
  ...
  void drawValues()
  {
    int xBegin = lvx-xOffset;
    int xEnd = viewPortW+lvx-xOffset;
    linePoint currentDataSet;
    drawYValues();
    drawXValues();
    ...
    {  
      currentDataSet=(linePoint )dataSetList.get(ds);
      float xmin = currentDataSet.getXPoint(0);
      float xmax = currentDataSet.getXPoint(currentDataSet.getSize()-1);
      float scale = (xEnd-xBegin)/(xmax-xmin);
      float incr = currentDataSet.getSize()/lNsamples;
      ...
      if (currentDataSet.visible()) {
        for (float i=0; i<currentDataSet.getSize(); i+=incr)
        {
          if ((int)i==(int)(i-incr)) continue;
          int j = (int) i;

          y2 = (float) currentDataSet.getYPoint(j);
          x2 = (float) currentDataSet.getXPoint(j);

          if (j>0)
          {
            float y1 = (float) currentDataSet.getYPoint(j-(int)incr);
            float x1 = (float) currentDataSet.getXPoint(j-(int)incr);
            drawPoint(y1, scale*(x1-xmin)+xBegin, y2, scale*(x2-xmin)+xBegin, ds);
          }  
          else 
          {
            drawPoint(y2, scale*(x2-xmin)+xBegin, ds);
          }
          currentDataSet.checkMinMax();
          //drawMinMax(y2,scale*(x2-xmin)+xBegin,ds,j);
        }
        float y3 = (float) currentDataSet.getYPoint(currentDataSet.getSize()-1);
        float x3 = (float) currentDataSet.getXPoint(currentDataSet.getSize()-1);
        drawPoint(y2, scale*(x2-xmin)+xBegin, y3, scale*(x3-xmin)+xBegin, ds);
      }
    }
  }
  /////////////////////////////////////////////////////////  
